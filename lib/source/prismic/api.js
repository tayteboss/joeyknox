...
export async function getAllWork() {
	...
	if (data.allProjects.pageInfo.hasNextPage) {
		...
		const fetchPaginationData = await fetchAPI(query, variables);
		paginationData = fetchPaginationData.allProjects.edges;
	}

	const mergeResult = [...data.allProjects.edges, ...paginationData];
